...
db.exec(`CREATE TABLE products(id TEXT PRIMARY KEY,name TEXT,category TEXT,subcategory TEXT,tone TEXT,price INTEGER,stock INTEGER,sold_count INTEGER,weight_grams INTEGER,description TEXT,material TEXT,care_instructions TEXT,production_estimate TEXT,size_guide TEXT,variants_json TEXT,image_key TEXT,image_url TEXT,images_json TEXT,active INTEGER,created_at TEXT,updated_at TEXT,deleted_at TEXT);CREATE TABLE admin_users(user_id TEXT);INSERT INTO admin_users VALUES('test');`);
const files=new Map();
...
const api=load('app/api/admin/products/route.ts',{'next/server':{NextResponse:{json:(body,options)=>Response.json(body,options)}},'@/app/chatgpt-auth':{getChatGPTUser:async()=>({userId:'test'})},'@/db':{getD1:()=>d1,getFiles:()=>({put:async(key,value)=>files.set(key,value)})},'@/lib/product-editor':lib,'@/lib/catalog-normalize':{normalizeCategory:v=>v,normalizeSubcategory:v=>v}});
...
const edit=form();edit.set('id',result.id);edit.set('galleryOrder',JSON.stringify(['n:0','s:1']));edit.append('images',new File(['three'],'three.jpg',{type:'image/jpeg'}));
...
const reopened=(await (await api.GET()).json()).products[0];assert.equal(reopened.images.length,2);assert.equal(reopened.images[1],lib.productImageUrl(originalKeys[1]));assert.ok(!reopened.images.includes(lib.productImageUrl(originalKeys[0])));assert.equal(reopened.image,reopened.images[0]);assert.equal(JSON.stringify(reopened.variants),JSON.stringify(generated));assert.equal(reopened.stock,75);assert.equal(reopened.price,54000);
