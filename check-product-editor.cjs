...
const api=load('app/api/admin/products/route.ts',{'next/server':{NextResponse:{json:(body,options)=>Response.json(body,options)}},'@/lib/admin-auth':{authorizeStore:async()=>({ok:true,admin:{store:{id:'default'}}}),adminCan:()=>true},'@/lib/audit':{audit:async()=>{}},'@/lib/tenant':{storeFileKey:(store,folder,ext)=>`stores/${store}/${folder}/${crypto.randomUUID()}.${ext}`},'@/db':{getD1:()=>d1,getFiles:()=>({put:async(key,value)=>files.set(key,value)})},'@/lib/product-editor':lib,'@/lib/catalog-normalize':{normalizeCategory:v=>v,normalizeSubcategory:v=>v}});
