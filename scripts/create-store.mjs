...
const domainArg = args
  ...
  ?.slice('--domain='.length);
const [slugArg, nameArg, emailArg, password, ...ownerParts] = args.filter(
  (arg) => !arg.startsWith('--'),
);
...
try {
  ...
  if (
    (await client.query('SELECT 1 FROM stores WHERE slug = $1', [slug])).rows[0]
  )
    throw new Error(
      `Slug "${slug}" sudah dipakai toko lain. Untuk menambah admin: npm run admin:create -- ... --store=${slug}`,
    );
  if (
    domain &&
    (
      await client.query('SELECT 1 FROM store_domains WHERE host = $1', [
        domain,
      ])
    ).rows[0]
  )
  ...
  const existing = (
    await client.query(
      'SELECT user_id, password_hash FROM admin_users WHERE email = $1',
      [email],
    )
  ).rows[0];
  ...
  if (!existing) {
    ...
    await client.query(
      'UPDATE admin_users SET password_hash = $2, updated_at = $3 WHERE user_id = $1',
      [userId, await hashPassword(password), now],
    );
  }
  await client.query(
  ...
  await client.query(
    ...
    [
      storeId,
      JSON.stringify({ slug, name, owner: email, via: 'store:create' }),
      now,
    ],
  );
  ...
  if (!existing) console.log(`✔ Akun pemilik ${email} dibuat.`);
    ...
    console.log(`  Akun ${email} sudah ada — password lama tetap dipakai.`);
  ...
  if (root && root !== 'localhost')
    console.log(`    https://${slug}.${root}/admin`);
  console.log(
    `    http://${slug}.localhost:3000/admin  (lokal, bila PLATFORM_ROOT_DOMAIN=localhost)`,
  );
} catch (error) {
  await client.query('ROLLBACK').catch(() => {});
  console.error('✖ Gagal:', error.message);
  if (/relation .* does not exist/.test(error.message))
    console.error('  Jalankan dulu: npm run db:migrate');
  process.exitCode = 1;
} finally {
  client.release();
  await pool.end();
