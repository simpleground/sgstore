...
export function VariantEditor({initial=[],preorderEnabled=false,preorderDays=2}:{initial?:EditorVariant[];preorderEnabled?:boolean;preorderDays?:number}) {
  const [preorder,setPreorder]=useState(preorderEnabled);
  ...
  function apply() {
    try {setRows(applyVariantBulk(rows,scope==='selected'?selected:null,price,preorder?'':stock));setError('');} catch(error){setError((error as Error).message);}
  }
  return <div className="rounded-xl border bg-white p-5 sm:col-span-2">
    ...
    <input type="hidden" name="preorder_enabled" value={preorder?'1':'0'}/>
    <section className="mb-5 rounded-lg border bg-slate-50 p-4">
      <label className="flex items-center justify-between gap-3 font-semibold">Produk pre-order<Switch checked={preorder} onCheckedChange={setPreorder}/></label>
      <label className="mt-4 block text-sm">Waktu persiapan setelah pembayaran (hari)<Input name="preorder_days" type="number" min={1} max={365} required defaultValue={preorderDays} className="mt-2 max-w-40"/></label>
      <p className="mt-2 text-sm text-slate-600">{preorder?'Stok tidak perlu diisi. Semua varian tersedia sebagai pre-order.':'Varian dengan stok 0 otomatis menjadi pre-order menggunakan waktu persiapan di atas.'} Waktu ini belum termasuk perjalanan kurir.</p>
    </section>
    ...
    <div className="my-5 grid gap-3 rounded-lg bg-slate-50 p-4 sm:grid-cols-2"><label className="text-sm">Harga jual massal<Input type="number" min="1" step="1" value={price} placeholder="Kosongkan jika tidak diubah" onChange={e=>setPrice(e.target.value)}/></label><label className="text-sm">Stok massal<Input type="number" min="0" step="1" disabled={preorder} value={stock} placeholder="Kosongkan jika tidak diubah" onChange={e=>setStock(e.target.value)}/></label><select aria-label="Sasaran penerapan massal" value={scope} onChange={e=>setScope(e.target.value)} className="rounded-lg border bg-white p-2"><option value="all">Semua varian ({rows.length})</option><option value="selected">Varian dipilih ({selected.length})</option></select><Button type="button" disabled={!rows.length||(scope==='selected'&&!selected.length)||(price===''&&stock==='')} onClick={apply}>Terapkan harga / stok</Button><p className="text-xs text-slate-500 sm:col-span-2">Harga massal menjadi harga normal dan harga jual; diskon pada varian sasaran direset ke 0%.</p></div>
